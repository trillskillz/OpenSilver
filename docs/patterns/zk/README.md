...
1. **Drive / amend the upstream patch path** in `kaspanet/silverscript`. OpenSilver contributed the initial builtin-exposure PR as `kaspanet/silverscript#125`, but local prototyping found that a **0-arg builtin alone is not sufficient** in current SilverScript syntax: the parser rejects raw expression statements (`a; b; 2; proof; vk;`) that would be needed to push operands before `OpZkPrecompile()`. So the real compiler-side unblock likely needs either expression statements / explicit push syntax, or a higher-arity builtin surface that lowers operands in canonical order.
2. **Use the local experimental patch lane** — OpenSilver now ships `patches/silverscript-opzkprecompile.patch` plus `npm run patch:silverc:zk`, which applies the RFC patch to the pinned upstream checkout, rebuilds `silverc`, and smoke-tests a minimal `.sil` containing `require(OpZkPrecompile())`. Good for probing compiler behavior; not yet enough for a real Phase-5 contract.
