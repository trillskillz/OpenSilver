...
- Compile suite: `npm run verify` (= `tsc -b` + `vitest`) is **23/23 files green, 32/32 tests** — one compile test per Phase-3 pattern, token scaffolds for 4.1 + 4.2 + 4.3 + 4.4 + 4.5, the shared manifest test, and SDK/integration tests covering KCC20 controller-path selection, constructor-arg builders, lifecycle planning, transaction-shape planning, compile/deploy spec bundles, a real TS-side `silverc` wrapper execution path, deploy-ready compiled flow assembly, broadcast-ready transaction assembly inputs, Kaspa-facing transaction-package mapping, RPC-backed UTXO resolution/build-request preparation, concrete Generator/PendingTransaction-based stage execution, and direct `kaspa-wasm` module binding for addresses/private keys/payment outputs.
- Runtime suite: `npm run test:runtime` (= `cargo test --manifest-path runtime-tests/Cargo.toml`) is **53/53 green, 0 ignored**. Each test compiles a `.sil` contract via `silverscript-lang` and executes the redeem script in `kaspa-txscript`'s `TxScriptEngine` against a hand-built `MutableTransaction` + `UtxoEntry`. Pairs are (happy path → engine OK, failure mode → `VerifyError|EvalFalse|UnsatisfiedLockTime`). Every Phase-3 core pattern is now end-to-end runtime-verified on its primary paths: TimeLock (claim/cancel/extend), Vault (release/extend/reconfigure), BilateralEscrow (release/timeout), milestone Escrow (KIP-20 cov-id continuation), Streaming Payment (withdraw/cancel), Vesting (claim/revoke), HTLC (claim/refund), MultiSig (2-of-3 threshold), DMS (ping/claim via CSV), FreelancePayroll (4 paths), Ownable (propose/accept), SocialRecovery (initiate/cancel). Phase 4 runtime coverage now includes **KCC20Capped** (controller init/asset binding + happy-path mint + over-cap rejection), **KCC20Pausable** (pause/unpause + paused-mint rejection), **KCC20Ownable** (pending-transfer mint, accepted-admin mint, stale-admin rejection), and **KCC20Vesting** (pre-cliff rejection, first scheduled mint, second-period mint, and final-drain mint). The SDK now also exposes KCC20 helper glue for controller-path lookup, constructor-arg building, state normalization, three-phase lifecycle planning, concrete transaction-shape plans, compile/deploy spec bundles targeting the pinned `silverc` path, a real TS-side compiler wrapper, deploy-ready compiled flow assembly, and broadcast-ready transaction assembly inputs; the `integrations/` package now maps those into Kaspa-facing transaction packages, resolves them through documented direct-node RPC UTXO lookup/subscription surfaces into builder-ready stage requests, executes those requests through a concrete Generator/PendingTransaction signing adapter, and binds that adapter to the real `kaspa-wasm` npm package for address/private-key/payment-output objects. **Still honest limitation:** covenant-bound outputs are now resolved to real Kaspa addresses at the adapter boundary, but we are not yet materializing custom script/covenant output fields from compiled `silverc` artifacts.
