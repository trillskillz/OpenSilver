...
2. **Use the local experimental patch lane** — `npm run patch:silverc:zk` applies `patches/silverscript-opzkprecompile.patch` to the pinned upstream checkout, rebuilds `silverc`, and smoke-tests both tracked contracts: `contracts/zk/opzkprecompile-smoke.sil` and `contracts/zk/opgroth16verify-smoke.sil`. Use this for local compiler probing and future prototyping while the upstream authoring surface is clarified.
...
3. ✅ **5.2 Private Asset Transfer** — DONE 2026-05-24 (v1 covenant-side). `contracts/zk/private-asset-transfer.sil` compiles via the patch lane; three runtime tests prove (valid proof + pinned output → OK), (wrong commitment_root → reject), (wrong recipient → reject). v1 honestly-documented limitations: no on-chain nullifier accumulator (single-use per deployment), amount as deploy-time state (not extracted from public inputs), no x-only-point validity gate on `pi_recipient` inside the covenant (circuit author's job). The COVENANT half is meaningfully tested; the CIRCUIT half is the deployment author's responsibility.
4. ✅ **5.4 Proof-Stitched Multi-Pattern** — DONE 2026-05-24. v1 contract at `contracts/zk/proof-stitched-multi-pattern.sil` compiles via the patch lane; three multi-input runtime tests (`proof_stitched_leader_delegate_two_input_batch_passes`, `_delegate_at_leader_position_rejected`, `_leader_rejects_tampered_proof`) prove the KIP-20 leader/delegate split: leader runs `OpGroth16Verify` once, delegate trusts via shared cov-context. New harness helper `execute_multi_input_with_covenants` makes multi-input cov-context tests tractable. v1 routes all inputs to one deploy-time recipient; per-recipient public-input decoding requires a real per-recipient circuit and is deferred.

### Phase 5 remaining

**All four core patterns now scaffolded + runtime-verified.** Remaining work is depth, not breadth:

1. **Real circuits for 5.2 + 5.4.** v1 covenants are wired against the existing Groth16 fixture (which acts as a placeholder). Real deployments need per-pattern circuits: 5.2 a commitment+nullifier transfer-validity circuit, 5.4 a per-recipient batch circuit. These are CIRCUIT-AUTHOR deliverables, not OpenSilver deliverables — but the design docs should grow concrete circuit examples as they land.
2. **On-chain nullifier accumulator for 5.2 v2.** Stateful continuation that tracks spent nullifiers via singleton transition. Needs the proof to bind `new_root = insert(old_root, nullifier)`, which depends on a real circuit.
3. **Per-recipient public-input decoding for 5.4 v2.** Pulls `(recipient_i, amount_i)` pairs from `public_inputs_concat` slices. Again needs a real per-recipient circuit.
4. **Cross-contract output binding for 5.3 v2.** Ties the oracle's published value into a consumer covenant via `validateOutputStateWithTemplate`. Implementable autonomously but better done in lockstep with a concrete consumer covenant.
5. **Upstream PR kaspanet/silverscript#125 fix-up.** Fold the stack-order `.rev()` correction back into the PR before it merges. User-gated.
