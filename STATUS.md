...
- New positive prototype result: a local `OpGroth16Verify(vk, proof, [a, b, c, ...])` helper that custom-flattens an array literal into verifier operands compiled successfully, and the upstream `silverscript-lang` `examples_tests` suite still passed (`26/26`).
