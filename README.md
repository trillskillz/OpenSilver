...
- Runtime suite: `npm run test:runtime` (= `cargo test --manifest-path runtime-tests/Cargo.toml`) is **30/30 green** (plus 7 documented-skipped pending compiler-gap fixes). Each test compiles a `.sil` contract via `silverscript-lang` and executes the redeem script in `kaspa-txscript`'s `TxScriptEngine` against a hand-built `MutableTransaction` + `UtxoEntry`. Pairs are (happy path → engine OK, failure mode → `VerifyError|EvalFalse|UnsatisfiedLockTime`). Covers TimeLock claim/cancel/extend, Vault release/extend/reconfigure composition, BilateralEscrow release + timeout, milestone Escrow KIP-20 continuation, HTLC claim/refund, MultiSig 2-of-3, DMS ping, and four FreelancePayroll paths. See `STATUS.md` for the per-pattern matrix and the three compiler-gap findings (return-must-be-last, NUM2BIN-on-byte[32]-state-writes, missing DAA-score harness plumbing).
