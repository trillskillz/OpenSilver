...
PHASE_5_STATUS: PATTERNS 5.1 + 5.3 LIVE LOCALLY (verified-computation.sil and zk-verified-oracle.sil both compile via npm run patch:silverc:zk and runtime-verify real Groth16 proofs through kaspa-txscript; 5.2/5.4 still design-only; upstream PR kaspanet/silverscript#125 still OPEN — local patch lane carries a stack-order correctness fix that needs folding back before merge)
...
TESTS_PASSING: 466/466 upstream + 29/29 vitest files (73/73 tests) + 64/64 cargo runtime suite (51 core + 7 kcc20 + 6 zk, 0 ignored)
