...
- **Phase 5 — ZK-aware (4):** Verified Computation, Private Asset Transfer, ZK-Verified Oracle, Proof-Stitched Multi-Pattern. **Design-only upstream, with local unblock tooling now in place**: four pattern designs landed in `docs/patterns/zk/`, each carrying state layout, intended `.sil` shape, public-inputs schema, cost amortisation table (5.4), and "WHEN NOT TO USE THIS" section. The engine-side opcode (`0xa6`) is fully shipped via `kaspanet/rusty-kaspa#775`, and OpenSilver has opened the front-end exposure PR as `kaspanet/silverscript#125`, but local prototyping found that a **0-arg builtin alone is not enough** for real contract authoring in current SilverScript syntax because raw operand-push statements are rejected by the parser. We also confirmed that generic array args are serialized as blob data, not separate stack items, while fixed higher-arity lowering does work mechanically. The newest prototype is more promising: a tag-specific `OpGroth16Verify(vk, proof, [a, b, c, ...])` helper with **custom array-literal flattening** compiled successfully in the local `silverscript` branch, and the full upstream `examples_tests` suite still passed. OpenSilver therefore ships `npm run patch:silverc:zk` mainly as a compiler-probing lane plus `buildGroth16WitnessPlan()` in the SDK to pin canonical Groth16 stack order while the upstream authoring surface is clarified.
