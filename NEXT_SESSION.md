...
1. **Resolve the SilverScript authoring surface around `OpZkPrecompile`**. `kaspanet/silverscript#125` exposes the builtin name, but local prototyping showed that a 0-arg builtin is not enough: the parser rejects raw operand-push statements like `a; b; 2; proof; vk;`. We also now know a higher-arity surface is mechanically viable because a local 9-arg fixed-5-input Groth16 prototype compiled cleanly. Track discussion on the PR and determine whether the right fix is expression statements / explicit push syntax or a structured higher-arity lowering surface.
