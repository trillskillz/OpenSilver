...
- **Phase 5 — ZK-aware (4):** Verified Computation, Private Asset Transfer, ZK-Verified Oracle, Proof-Stitched Multi-Pattern. **5.1 + 5.3 are scaffolded + runtime-verified locally**: `contracts/zk/verified-computation.sil` and `contracts/zk/zk-verified-oracle.sil` compile via `npm run patch:silverc:zk` and six runtime tests in `runtime-tests/tests/zk_runtime.rs` prove real Groth16 fixtures verify through `kaspa-txscript`'s engine end-to-end. 5.3 specifically composes 5.1's Groth16 surface with the MultiSig-style M-of-N committee threshold (2-of-3 + valid proof = OK, below-threshold = reject, tampered proof = reject). The patch lane carries a stack-order correctness fix that needs folding into upstream PR `kaspanet/silverscript#125` before merge. The chosen authoring surface is the tag-specific `OpGroth16Verify(vk, proof, [pi0, pi1, ...])` helper with custom array-literal flattening. 5.2/5.4 still design-only in `docs/patterns/zk/`.
