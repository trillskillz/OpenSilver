...
describe('Milestone Escrow contract scaffold', () => {
  it('parses with silverc and exposes milestone progression/release/refund entrypoints in the AST', () => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), 'opensilver-escrow-milestone-'));
    const output = join(tempDir, 'escrow-milestone-ast.json');
    ...
    try {
      ...
      expect(printed).toContain('approve_milestone');
