...
- Runtime suite: `npm run test:runtime` (= `cargo test --manifest-path runtime-tests/Cargo.toml`) is **34/34 green** (plus 5 documented-skipped pending the NUM2BIN compiler-gap fix). Each test compiles a `.sil` contract via `silverscript-lang` and executes the redeem script in `kaspa-txscript`'s `TxScriptEngine` against a hand-built `MutableTransaction` + `UtxoEntry`. Pairs are (happy path → engine OK, failure mode → `VerifyError|EvalFalse|UnsatisfiedLockTime`). Covers 13 patterns: TimeLock claim/cancel/extend, Vault release/extend/reconfigure composition, BilateralEscrow release + timeout, milestone Escrow KIP-20 continuation, HTLC claim/refund, MultiSig 2-of-3, DMS ping, Streaming cancel, Vesting revoke, and four FreelancePayroll paths. See `STATUS.md` for the per-pattern matrix and the remaining two compiler-gap findings (NUM2BIN-on-byte[32]-state-writes and missing DAA-score harness plumbing — the earlier return-must-be-last gap is now closed via single-return refactor of streaming-payment.sil + vesting.sil).
