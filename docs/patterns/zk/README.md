...
1. **Land an upstream patch** in `kaspanet/silverscript` adding the `OpZkPrecompile` builtin to `silverscript-lang::std::builtins.sil` with the canonical stack shape Saefstroem's verifier consumes (uncompressed VK, compressed proof, `n_inputs: i32`, then n public inputs). This is a roughly one-screen change to the compiler. OpenSilver has now contributed it: tracking issue #3, upstream PR `kaspanet/silverscript#125`.
