...
- **Phase 5 — ZK-aware (4):** Verified Computation, Private Asset Transfer, ZK-Verified Oracle, Proof-Stitched Multi-Pattern. **5.1 + 5.3 + 5.4 are scaffolded + runtime-verified locally**: three contracts under `contracts/zk/` compile via `npm run patch:silverc:zk` and nine runtime tests in `runtime-tests/tests/zk_runtime.rs` prove real Groth16 fixtures verify through `kaspa-txscript`'s engine end-to-end. 5.3 composes 5.1's Groth16 surface with the MultiSig-style M-of-N committee threshold. **5.4 demonstrates the KIP-20 leader/delegate cost-amortisation pattern**: two inputs sharing one covenant_id, where the leader runs `OpGroth16Verify` once and the delegate skips verification via shared cov-context. New harness helper `execute_multi_input_with_covenants` makes multi-input cov-context tests tractable. The patch lane carries a stack-order correctness fix that needs folding into upstream PR `kaspanet/silverscript#125` before merge. Only 5.2 (Private Asset Transfer) is still design-only — it needs a real commitment/nullifier circuit before its covenant becomes meaningful.
