...
- **Phase 5 — ZK-aware (4):** Verified Computation, Private Asset Transfer, ZK-Verified Oracle, Proof-Stitched Multi-Pattern. **5.1 Verified Computation is now scaffolded + runtime-verified locally**: `contracts/zk/verified-computation.sil` compiles via `npm run patch:silverc:zk` and three runtime tests in `runtime-tests/tests/zk_runtime.rs` prove a real Groth16 fixture verifies through `kaspa-txscript`'s engine end-to-end (positive + tampered-proof + wrong-prover). The patch lane carries a stack-order correctness fix discovered via real fixture testing that needs folding into upstream PR `kaspanet/silverscript#125` before merge. The engine-side opcode (`0xa6`) is fully shipped via `kaspanet/rusty-kaspa#775`. The chosen authoring surface is the tag-specific `OpGroth16Verify(vk, proof, [pi0, pi1, ...])` helper with custom array-literal flattening — proven to compile cleanly and lower correctly. 5.2/5.3/5.4 still design-only in `docs/patterns/zk/`; implementation order documented in `NEXT_SESSION.md`.
